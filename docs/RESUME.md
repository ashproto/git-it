...
**✅ REDESIGN COMPLETE (R1–R4)** + a new **"Graph tick+" app icon** (`f34c921`). **Post-redesign, also DONE & merged** (HEAD `536df38`; 89 Rust + 107 Vitest green; resume-handoff lives in the `gitit-backlog.md` memory file): a proper **Settings panel** (Curved/Angular + Tabs/Sidebar toggles moved in; old gear popover retired) + **relative "Today/Yesterday" commit dates** (default on); cleanups (`RepoLoader.svelte` deleted, graph geometry centralized — `6e2d5fe`); and **3 of the 4 optional features** — **squash-merge UI** (`ee21af5`), **interactive-rebase `edit`** pause/amend (`9f645ba`), **line-level (intra-hunk) staging** (`536df38`). A `tauri build` was delivered to the user's ~/Desktop. **THE ONE REMAINING ITEM: DOM virtualization** (frontend-only GraphHistory/GraphGutter row+SVG windowing for huge histories — design, invariants-to-preserve, and a preview-seed note are in `gitit-backlog.md`'s "HANDOFF — RESUME HERE" section). After that the app is feature-complete. Manual (user): create the `<user>/git-it` GitHub repo (+ a Homebrew tap if distributing — `git-it`/`gitsome` are taken on npm/PyPI but a Tauri `.dmg` publishes to neither).
